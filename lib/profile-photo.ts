...
import { cropRectangle, type PhotoCrop } from './photo-crop';
...
export async function prepareProfilePhoto(file: File, crop?: PhotoCrop): Promise<Blob> {
  ...
  try {
    ...
    const rect = crop ? cropRectangle(bitmap.width, bitmap.height, crop)
